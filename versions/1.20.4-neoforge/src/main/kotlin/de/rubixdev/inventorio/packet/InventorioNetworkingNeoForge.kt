...
object InventorioNetworkingNeoForge : InventorioNetworking {
    ...
    fun registerPayloads(event: RegisterPayloadHandlerEvent) {
        ...
        registrar.play(SelectUtilitySlotPacket.identifier, ::SelectUtilitySlotPacket) { handler ->
            handler.client(SelectUtilitySlotPacket::consumeClient).server(SelectUtilitySlotPacket::consumeServer)
        }
        ...
        registrar.play(UseBoostRocketC2SPacket.identifier, { UseBoostRocketC2SPacket() }) { handler -> handler.server(UseBoostRocketC2SPacket::consume) }
        ...
        registrar.play(OpenInventorioScreenC2SPacket.identifier, { OpenInventorioScreenC2SPacket() }) { handler -> handler.server(OpenInventorioScreenC2SPacket::consume) }
        registrar.play(SwapItemsInHandsKeyC2SPacket.identifier, { SwapItemsInHandsKeyC2SPacket() }) { handler -> handler.server(SwapItemsInHandsKeyC2SPacket::consume) }
    }
