...
public final class InventorioAPI
{
     ...
     *
     * Note: each tick handler is run within its own try-catch block.<br>
     *
     ...
     *
     * @return If <code>slotName</code> has already been taken, returns the existing {@link ToolBeltSlotTemplate}.
     *
     * Creates a new one and returns it otherwise.
     *
     * @throws IllegalStateException when attempted to add a new Template post initialization.
    ...
    {
        return PlayerScreenHandlerAddon.registerToolBeltTemplateIfNotExists(slotName, new ToolBeltSlotTemplate(slotName, emptyIcon));
    }
    ...
    {
        return PlayerScreenHandlerAddon.getToolBeltTemplate(slotName);
    }
    ...
    {
        return PlayerScreenHandlerAddon.getToolBeltTemplates();
    }
    ...
    @Nullable
    public static PlayerScreenHandlerAddon getScreenHandlerAddon(@NotNull PlayerEntity playerEntity)
    {
        return PlayerScreenHandlerAddon.getScreenHandlerAddon(playerEntity);
    }
}
