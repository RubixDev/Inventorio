...
abstract class AbstractInventorioScreenHandler protected constructor(syncId: Int, val inventory: PlayerInventory) :
    AbstractRecipeScreenHandler<CraftingInventory?>(ScreenTypeProvider.INSTANCE.getScreenHandlerType(), syncId) {
    protected val inventoryAddon = inventory.player.inventoryAddon!!
    ...
    companion object {
        ...
        private val openConsumers = mutableMapOf<Identifier, Consumer<AbstractInventorioScreenHandler>>()
        private val armorSlots = arrayOf(EquipmentSlot.HEAD, EquipmentSlot.CHEST, EquipmentSlot.LEGS, EquipmentSlot.FEET)

        @JvmStatic
        fun registerOpenConsumer(customIdentifier: Identifier, screenHandlerConsumer: Consumer<AbstractInventorioScreenHandler>) {
        ...
        fun open(player: PlayerEntity) {
            player.openHandledScreen(
                SimpleNamedScreenHandlerFactory(
                    { syncId, playerInventory, _ -> InventorioScreenHandler.create(syncId, playerInventory!!) },
