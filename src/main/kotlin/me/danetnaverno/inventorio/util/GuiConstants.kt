...
fun GUI_PLAYER_INVENTORY_EXTENSION_WITH_DECO_P1(extensionRowCount: Int) = Rectangle(
        -13, 70,
        256, 13 + extensionRowCount * INVENTORY_SLOT_SIZE)

fun GUI_PLAYER_INVENTORY_EXTENSION_WITH_DECO_P2(extensionRowCount: Int) = Rectangle(
        -13, 83 + extensionRowCount * INVENTORY_SLOT_SIZE,
        256, 17)
...
fun GUI_EXTERNAL_INVENTORY_EXTENSION_PART_BOTTOM(extensionRowCount: Int) = Rectangle(
        -13, 17 + extensionRowCount * INVENTORY_SLOT_SIZE,
        256, 13 + 4)

fun GUI_EXTERNAL_INVENTORY_MAIN_PART(extensionRowCount: Int) = Rectangle(
        -13, 17 + if (extensionRowCount <= 0) 0 else (4 + extensionRowCount * INVENTORY_SLOT_SIZE),
        256, 96)
...
fun GUI_EXTERNAL_INVENTORY_QUICK_BAR(extensionRowCount: Int) = Rectangle(
        7, 141 + GUI_EXTERNAL_INVENTORY_EXTENSION_PART(extensionRowCount).height,
        216, 18)
